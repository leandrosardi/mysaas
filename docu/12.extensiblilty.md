...
You can also define the working folder for your dependencies.
...
BlackStack::Extensions::add(
    ...
    :settings => {
        ...
        # what are the screens to add in the leftbar
        :leftbar => [
            # add an entry with the label "dashboard`, with the icon `icon-dashboard`, and poiting to the scren `helpdesk/dashboard`. 
            { :label => 'Invoices', :icon => :dashboard, :screen => :invoices, },
            # add an icon with the label "tickets`, with the icon `icon-envelope`, and poiting to the scren `helpdesk/tickets`. 
            { :label => 'PayPal Subscriptions', :icon => :envelope, :screen => :subscriptions, },
            # add an icon with the label "tickets`, with the icon `icon-envelope`, and poiting to the scren `helpdesk/tickets`. 
            { :label => 'Transactions', :icon => :envelope, :screen => :transactions, },
        ],
    },
)
```
